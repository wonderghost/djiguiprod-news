      ...
      <v-container>
         <div class="text-right" id="form">
               <v-row>
                  ...
                  <v-col cols="12" md="4">
                     <v-text-field label="Nom de la catégorie" name="name" required type="text" color="blue darken-2" :error-messages="errors.name" v-model="form.name"></v-text-field>
                  </v-col>

                  <v-col cols="12" md="2">
                     <v-btn rounded dark color="primary" class="mb-5" @click="addCategory()" :loading="isLoading">
   ...
   methods: {
      addCategory: async function () {
         ...
         try {
            ...
            let response = await axios.post('/request/category/store', this.form)
