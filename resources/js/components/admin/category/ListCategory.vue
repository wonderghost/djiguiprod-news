         ...
         <tbody>
               <tr v-for="(category, index) in categories" :key="index">
                  ...
                  <td class="text-center">
                     <p>
                           <v-icon color="success" class="mx-3" @click="openEditDialog(category)">mdi-circle-edit-outline</v-icon>
                           <v-icon color="error" @click="openDialog(category)">mdi-delete</v-icon>
                     </p>
                  </td>
               </tr>
         </tbody>
      ...
      <v-dialog v-model="editDialog" persistent max-width="350">
         <v-card>
               ...
               <v-card-text>
                  <v-container>
                     <v-row>
                           <v-col cols="12">
                              <v-form>
                                 <v-text-field label="Nom de la catégorie" :value="singleEditCategory.name" v-model="singleEditCategory.name" required>
                                 </v-text-field>
                              </v-form>
                           </v-col>
                     </v-row>
               ...
               <v-card-actions>
                  ...
                  <v-btn color="blue darken-1" text @click="editCategory()" :loading="isLoading">
                     Modifier
                  </v-btn>
               </v-card-actions>
         </v-card>
   ...
   methods: {
      editCategory: async function () {
         ...
         try {
            ...
            let response = await axios.put("/request/category/" + this.singleEditCategory.slug + "/update",
