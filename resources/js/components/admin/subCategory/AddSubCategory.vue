    ...
    <v-form ref="form">
        <v-container>
            <div class="text-right" id="form">
                <v-row>
                    <v-col cols="12" md="6">
                        <v-text-field label="Nom de la sous-catégorie" name="name" required type="text" color="blue darken-2" v-model="form.name"></v-text-field>
                    </v-col>

                    <v-col cols="12" md="4">
                        <v-autocomplete v-model="form.id_category" :items="categories" label="Categorie" item-text="name" item-value="slug"></v-autocomplete>
                    </v-col>

                    <v-col cols="12" md="2">
                        <v-btn rounded dark color="primary" class="mb-5" @click="addSubCategory()" :loading="isLoading">
   ...
   methods: {
      addSubCategory: async function () {
         ...
         try {
               ...
               let response = await axios.post("request/sub-category/store", this.form);
